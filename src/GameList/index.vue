...
import { categories, games, searchGames } from '../config/games.js'

const router = useRouter()

// 使用抽离的配置（转换为 ref 以保持响应式）
...
const stats = computed(() => ({
  total: gamesList.value.filter(g => !g.disabled).length,
  comingSoon: gamesList.value.filter(g => g.comingSoon).length
}))
...
onMounted(() => {
  ...
  analyticsTracker.trackPageView('game_list', {
    ...
    categories: categoryList.value.map(c => c.id)
  })
})
    ...
    <div class="main-content">
      ...
      <div class="sidebar">
        <div class="category-list">
          ...
          >
            <!-- 安全说明: category.icon 是内部静态 SVG 配置，非用户输入，XSS 风险可控 -->
        ...
        <div class="games-grid">
          ...
          >
            <div class="game-header">
              <!-- 安全说明: game.icon 是内部静态 SVG 配置，非用户输入，XSS 风险可控 -->
