...
<script setup>
  ...
  const startGame = () => {
    ...
    actions.value = [
      { text: '开始修炼', handler: () => router.push('/xiuxian/cultivate') },
      {
        text: '探索秘境',
        handler: () => {
      ...
      { text: '挑战无尽塔', handler: () => router.push('/xiuxian/endlesstower') },
      { text: '世界BOSS', handler: () => router.push('/xiuxian/boss') },
      { text: '休闲娱乐', handler: () => router.push('/xiuxian/game') }
    ]
    // 初始化玩家当前气血
